...
def test_expectation():
    '''
        checks whether 
            ...
            E[f] = E[g]*E[f1] + (1-E[g])*E[f0]

        holds when f is modular in g w/ cofactors f1 and f0
    '''
    for _ in range(100):
        f,h,f1,f0 = imp.random_module(X[:4], Y[:3])
        assert f.expectation() == h.expectation()*f1.expectation() + (1-h.expectation())*f0.expectation()
