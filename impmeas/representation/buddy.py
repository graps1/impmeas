...
def load(fil) -> BuddyNode:
	...
	# print(f"loaded {fil}")
	return BuddyNode(root.value)

def random_order(cnf):
	vs = { abs(x) for cl in cnf for x in cl }
	return list(vs)

def force_heuristic(cnf, c=2):
	# expect the cnf in a list-of-lists-format
	vs = random_order(cnf)
	old_vs = vs
	for _ in range(int(c*math.log(len(vs)))): 
		# preprocessing
		position_by_var = { x: idx for idx,x in enumerate(vs) }
		clauses_by_var = defaultdict(set)
		for idx, clause in enumerate(cnf):
			for lit in clause:
				clauses_by_var[abs(lit)].add(idx)

		# compute cog for every clause
		cogs = {}
		for idx, clause in enumerate(cnf):
			cog = sum(position_by_var[abs(lit)] for lit in clause)/len(clause)
			cogs[idx] = cog

		# compute new variable positions
		new_positions_by_var = { }
		for x in vs:
			cs = clauses_by_var[x]
			new_positions_by_var[x] = sum(cogs[i] for i in cs)/len(cs)
		vs = sorted(new_positions_by_var.keys(), key=lambda x: new_positions_by_var[x])
